...
static void sb_mut_tabs_build(Compiler *c, SbMutTab *lt, SbMutTab *it, int toplevel) {
  ...
  NT_FOREACH_KIND(nt, NK_CallNode, u) {
    ...
    signed char v = 0;
    /* `[]=` insert slice! setbyte have no ivar arm of their own, but the
       shared-mutable shim runs them over an ivar handle through a shadow
       (#4363), so they promote like any other mutator. Counting them as
       disqualifying kept the ivar a plain string for good, and a mutation
       through its reader elsewhere (`obj.name << x`) then landed in a copy. */
    if (sp_str_mutator(un, SP_MUT_IVAR) ||
        sp_streq(un, "insert") || sp_streq(un, "slice!") ||
        sp_streq(un, "[]=") || sp_streq(un, "setbyte")) v = 1;
...
static int promote_shared_stored_strings(Compiler *c) {
  ...
  for (int mu = comp_kind_first(c, NK_CallNode); mu >= 0; mu = comp_kind_next(c, mu)) {
    ...
    const char *mun = nt_str(nt, mu, "name");
    if (!mun) continue;
    /* every mutator: the ones with no ivar arm of their own run through the
       shared-mutable shim over the reader's handle */
    if (!sp_str_mutator(mun, SP_MUT_LOCAL)) continue;
    int mrecv = nt_ref(nt, mu, "receiver");
    if (mrecv < 0 || nt_kind(nt, mrecv) != NK_CallNode) continue;
