...
The String ivar only becomes a shared handle through the passes in `src/analyze.c`, and each one misses a case. `promote_shared_stored_strings` shares a reader alias (`x = c.name`) only when the ivar's own class mutates it, not when the only mutation is through the reader. The same function's pass for mutations through a reader only takes the mutators that have an ivar form (`SP_MUT_IVAR`), so `slice!` and `setbyte` through a reader are dropped (`[]=` and `insert` through a reader are #N). And `sb_mut_tabs_build` marks an ivar that its class mutates with `insert`, `slice!`, `[]=` or `setbyte` as never to be shared, so any later mutation through its reader lands in a copy.
