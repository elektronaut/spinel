...
This overlaps #N (`fix-super-anon-block-pf-name`), which fixes the same `m#pf` lookup by stripping the suffix in `comp_prep_user_name` and routing the inlined-super lookup in `emit_super_inline` through it. Each branch passes the other's test, so only one of them needs to merge. They merge without conflicts, and with both merged `comp_super_name` never sees a `#pf` name.
